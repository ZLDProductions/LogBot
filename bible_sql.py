...
sql = sqlite3.connect(f"{discord_settings}\\bible_plugin\\votd.db")
cursor = sql.cursor()
...
def trigger_votd():
	...
	if last_day != datetime.now().day:
		...
		key = random.choice(_read("""SELECT _key FROM verses;"""))[0]
		votd = key
		...
		for channel in client.get_all_channels():
			...
			if server.id in list(default_channel.keys()) and not server.id in encountered:
				...
				yield from client.send_message(channel, "Here is the VotD!", embed=e)
...
async def on_message(message):
	...
	if not message.server.id in verse_disable_list and not message.author.bot and not message.content.startswith(f"$verse ") and not message.channel.id in disabled_channels:
		...
		encountered = []
		tmp_content = message.content.replace("\n", " ").replace(".", " ")
		verse = []
		...
		if bible_types[message.author.id] == "embed" and len(myembed.fields) >= 1:
			try:
				list(tmp_content).index('"')
				# <editor-fold desc="Fetching Title...">
				index = tmp_content.find("\"")
				title = ""
				has_found = False
				for i in range(index + 1, len(tmp_content)):
					if tmp_content[i] == "\"": has_found = True
					if not has_found: title += tmp_content[i]
					pass
			...
			await client.send_message(message.channel, f"Response in {delay} seconds!", embed=myembed)
	...
	if startswith(f"$disable verse", f"$disable $verse"):
		if admin_role in message.author.roles or message.author.id == owner_id: verse_disable_list.append(message.server.id)
		else: sendNoPerm(message)
		pass
	elif startswith(f"$enable verse", f"$enable $verse"):
		if admin_role in message.author.roles or message.author.id == owner_id: verse_disable_list.remove(message.server.id)
		else: sendNoPerm(message)
		pass
	elif startswith(f"$verse help"):
		if (member_role in message.author.roles and not message.server.id in verse_disable_list) or message.author.id == owner_id:
		...
		if (member_role in message.author.roles and not message.server.id in verse_disable_list) or message.author.id == owner_id:
			...
			if not " " in cont:
				if cont == "Genesis":
					ret += "Genesis\nAuthor: {}\nChapters: {}\n{}".format(bi.getGenesisAuthor(), bi.getGenesisChapters(), bi.getGenesisSummary())
					pass
				elif cont == "Exodus":
					ret += "Exodus\nAuthor: {}\nChapters: {}\n{}".format(bi.getExodusAuthor(), bi.getExodusChapters(), bi.getExodusSummary())
					pass
				elif cont == "Leviticus":
		...
		if (member_role in message.author.roles and not message.server.id in verse_disable_list) or message.author.id == owner_id:
			...
			if bible_types[message.author.id] == "text":
				messages = format_message(verse)
				for item in messages: await client.send_message(message.channel, item)
				pass
			else:
				stuffs = verse.split("\n")
				myembed = discord.Embed(title=bible_versions[message.author.id], colour=discord.Colour.green())
				myembed.add_field(name=stuffs[0], value=stuffs[1])
				await client.send_message(message.channel, "Here you go!", embed=myembed)
				pass
			pass
		elif message.server.id in verse_disable_list: sendDisabled(message)
		else: sendNoPerm(message)
		...
		if (member_role in message.author.roles and not message.server.id in verse_disable_list) or message.author.id == owner_id:
			await client.send_typing(message.channel)
			c = message.content.replace(f"$verse search", "")
			page = 0
			if startswith(".", val=c): page = int(c.split(" ")[0].replace(".", ""))
			key = c.replace("." + str(page) + " ", "")
		...
		if (member_role in message.author.roles and not message.server.id in verse_disable_list) or message.author.id == owner_id:
			...
			if bible_types[message.author.id] == "text":
				...
				if "-" in content:
					...
					for item in stuffs: await client.send_message(message.content, "```" + item + "```")
					pass
				pass
			else:
				if "-" in content:
		...
		pass
	elif startswith(f"$setversion "):
		c = message.content.replace(f"$setversion ", "")
		if c == "kjv": bible_versions[message.author.id] = "kjv"
		elif c == "akjv": bible_versions[message.author.id] = "akjv"
		elif c == "web": bible_versions[message.author.id] = "web"
		await client.send_message(message.channel, f"Set version to {c}")
		pass
	elif startswith(f"$settype "):
		c = message.content.replace(f"$settype ", "")
		bible_types[message.author.id] = "text" if c == "text" else "embed"
		await client.send_message(message.channel, f"```Set the type to: {bible_types[ message.author.id ]}```")
		pass
	elif startswith(f"$setversion"):
		...
		if admin_role in message.author.roles:
			if "None" in message.content:
				...
				await client.send_message(message.channel, "Set the default channel None.")
				pass
			else:
		...
		pass
	elif startswith(f"v$disables"):
		ret = ""
		for _id in disabled_channels:
			c = client.get_channel(_id)
			if c.server == message.server: ret += ", " + str(c)
			pass
		try: await client.send_message(message.channel, ret[2:])
		...
		pass
	elif startswith(f"v$get\n", "```sql\n--verse-get"):
		if message.author.id == owner_id:
			try:
				cmd = message.content.replace("v$get\n", "").replace("```sql", "").replace("```", "")
				res = _read(cmd)
				for item in format_message(f"```Execution Successful. Result:\n{res}```"): await client.send_message(message.channel, item)
				pass
			except: await client.send_message(message.channel, "```Execution Failed.```")
			pass
		else: sendNoPerm(message)
		pass
	elif startswith(f"v$execute\n", "```sql\n--verse-execute"):
		if message.author.id == owner_id:
			try:
				cmd = message.content.replace("v$execute\n", "").replace("```sql", "").replace("```", "")
				_execute(cmd)
...
async def on_ready():
	...
	try: _execute(f"""
	CREATE TABLE verses (_key VARCHAR(20));
	""".replace("\t", ""))
	except: pass
	try: _execute(f"""
	CREATE INDEX vindex
